...
class NodeList extends React.Component<Props> {
  ...
  @computed get filteredNodeList() {
    ...
    const nodeString = node => `${node.moduleName}.${node.name} ${node.typeDisplay}`
    const match = node => !!nodeString(node).match(regex)
    return this.store.node.nodeList.filter(node => match(node)).slice(0, MAX_ITEMS)
  }
